...
namespace IWP.Generic {
    internal sealed class ObjPool: MonoBehaviour {
		...
		internal void InitMe(int size, GameObject prefab, Transform parentTransform, bool worldPosStays) {
			activeObjs = new List<GameObject>(size);
			inactiveObjs = new List<GameObject>(size);

			for(int i = 0; i < size; ++i) {
				GameObject GO = Instantiate(prefab, parentTransform, worldPosStays);
				GO.SetActive(false);
				inactiveObjs.Add(GO);
			}
		}
